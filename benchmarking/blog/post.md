...
*Attributing an answer to its sources after the fact — from an open model's attention, on any model's output — lands within a few points of frontier-LLM citation quality (F1 0.785 vs 0.81–0.85), at ~5–6× lower latency and ~7× lower cost.*
...
**TL;DR:** post-hoc attention attribution lands just behind generation-time methods on quality (F1 0.785 vs 0.81–0.85) and well ahead of naive retrieval (0.62), while being ~6× faster, ~7× cheaper, needing no document index, and working on any model's output.
...
| **TokenPath** *(post-hoc attention)* | 0.712 | 0.898 | **0.785** | 106 | **1.68 s** | **$0.0125** |
...
**1. Post-hoc attention is competitive on quality.** TokenPath (0.785) trails the generation-time Citations API (0.812) by 0.027 and the prompted LLM (0.851) by 0.066 — but it beats the published LongCite-8B + SelfCite anchor (0.782, different generations, context only) and crushes naive retrieval (0.622). Its **precision (0.898) is right alongside the LLM methods**.

**2. It's dramatically cheaper and faster.** ~1.7 s vs ~8–10 s (~6× faster), $0.013 vs ~$0.09 (~7× cheaper). And unlike the LLM methods, TokenPath's cost is *model-independent* — a flat $1 per 1M attributed tokens regardless of who wrote the answer — because it works post-hoc on any model's output.
...
The story is a Pareto frontier: the two LLM methods buy the top ~0.03–0.07 of F1 with 5–8× the latency and ~7× the cost. TokenPath sits at the knee — near-frontier quality, a fraction of the price. *(Chart data: `data/main_comparison.csv`.)*
...
|---|---|---|---|---|
| gov_report (summary) | 0.972 | 0.947 | 0.854 | 0.843 |
| hotpotqa (multi-hop QA) | 0.811 | 0.732 | 0.802 | 0.657 |
| longbench-chat (abstractive) | 0.769 | 0.758 | 0.699 | 0.367 |
| multifieldqa_en (QA) | 0.854 | 0.807 | 0.799 | 0.705 |

---

## Getting there: tuning the attention aggregation

TokenPath's raw output is a token×token attention heatmap. Turning that into sentence citations is an aggregation problem, and the naive version left a lot on the table. Our first cut scored **F1 0.723**. Three changes to how we read the heatmap took it to **0.785** — closing ~70% of the gap to the Citations API — with no change to the API call, latency, or cost:
...
**1. It doesn't win on raw quality — and the gap is real.** Post-hoc attention (0.785) is genuinely below generation-time citation (0.81–0.85). The gap concentrates on **abstractive/paraphrastic** statements: when the answer says *"appointed with Senate confirmation"* but the document says *"advice and consent of the Senate,"* an LLM reasons across the paraphrase; attention does surface matching and can miss it. This is why TokenPath is strongest on extractive QA (hotpotqa 0.80) and weakest on abstractive chat (longbench-chat 0.70). Some of this gap is likely a floor for pure attention attribution, not a tuning knob.

**2. TokenPath cites at sentence/passage granularity.** Its citations average 106 tokens (whole sentences, and merged passages) vs the prompted LLM's surgical 35-token quotes. It reliably points you to the right *place*; it doesn't pinpoint the exact clause. (The Citations API is coarser still, at 177 tokens.)
...
If you need the single highest-quality citations and don't mind a second frontier LLM call per query (or letting the model rewrite your answer), generation-time methods win by a few points of F1. But if you want **good citations on any model's output, cheaply, fast, without regenerating the answer or standing up a retrieval index** — post-hoc attention attribution is now within a hair of the frontier on quality and far ahead on everything else.
...
- TokenPath aggregation: `{threshold: 0.30, row_norm: true, merge_adjacent: true}`.
