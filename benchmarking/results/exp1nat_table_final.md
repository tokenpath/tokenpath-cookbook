...
> **Updated 2026-07-15** after the hosted API's reference-model upgrade
> (Llama-3.1-8B → Qwen3.5-9B). TokenPath re-run with the identical tuned
> aggregation config: F1 0.785 → **0.815**. Other methods unchanged. The
> pre-upgrade TokenPath row is kept below for the record.
...
| **TokenPath — tuned** *(post-hoc attention)* | 0.735 | 0.938 | **0.815** | 114 | 0.74 | **1.60s** | **$0.013** |
...
| *TokenPath — tuned, pre-upgrade Llama-3.1-8B reference model (archived)* | 0.712 | 0.898 | 0.785 | 106 | 0.79 | 1.68s | $0.0125 |
...
**Reading it**
- **Quality:** post-hoc LLM (prompted, 0.851) > **TokenPath (0.815)** ≥ generation-time (Citations API, 0.812) ≫ embedding (0.622). TokenPath's precision (0.938) is the highest of any method; the prompted LLM's remaining lead is recall.
...
**TokenPath tuning (original run):** aggregation `{threshold 0.30, row_norm, merge_adjacent}` (`config.TOKENPATH_AGG`), which took TokenPath from 0.723 → 0.785 reported-avg (0.754 → 0.810 example-mean) on the pre-upgrade reference model — see the sweep in `results/sweep/`. The 2026-07-15 re-run uses this exact config unchanged and scores 0.815.
